...
class BaseCheckMixin(object):
    ...
    # def logic_check(self,digits,data):
    #     for i in data:
    #         for j in digits:
    #             if not i[j].isdigit():
    #                 if j != 'RoomScale':
    ...
    #                     return False
    #     return True
    def volume_check(self,i):
        if int(i['Volume']) != (int(i['RoomScale'][0]) * 30):
            print i['RoomID'] +  '教室的列数与总容量不匹配'
            return False
        else:
            return True
    def damage_check(self,i):
        digits = ['Damage1','Damage2','Damage3','Damage4','Damage5']
        for j in digits:
            if int(i[j]) <0 or int(i[j]) > 30:
                print i['RoomID'] + '班的第' + j[-1] + '列错误'
                return False
        return True
